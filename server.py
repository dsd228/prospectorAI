...
def _best_ollama_model(task="general"):
    ...
    priority = {
        "creative": ["llama3.1:latest", "llama3:latest", "mistral:latest", "deepseek-r1:latest"],
        "code": ["deepseek-coder:latest", "codellama:latest", "llama3.1:latest"],
        "general": ["llama3.1:latest", "llama3:latest", "mistral:latest", "deepseek-r1:latest"],
        "fast": ["phi3:latest", "gemma:2b", "mistral:latest"]
    }

    for model in priority.get(task, priority["general"]):
        for installed in models:
            if model.split(":")[0] in installed or model == installed:
                return installed

    return models[0] if models else "deepseek-r1:latest"
...
def ai_generate():
    ...
    ollama_url = os.environ.get('OLLAMA_URL', '') or cfg.get('ollama_url', 'http://localhost:11434')
    model = _best_ollama_model(d.get('task', 'general'))
    try:
        ...
        return jsonify({'ok': False, 'error': f'Sin APIs disponibles: {str(e)}'})
